      SUBROUTINE RA15(X,V,TF,XL,LL,NV,NCLASS,NOR, UM)   
C  UM = the masses : UM(1) - mass of the first body etc
...
C  Integration is in double precision. A 64-bit double-word is assumed.
     ...
     A     B(7,18),G(7,18),E(7,18),BD(7,18),H(8),W(7),U(7),NW(8),UM(4)
...
C  values from the predicted B-values, following Eq. (2.7) in text.
 722  DO 58 K=1,NV
  ...
  58  G(7,K)=                                                   B(7,K)
      ...
      IF(NCL) T2=T
      TVAL=DABS(T)
CC      IF(NS/10*10.EQ.NS) WRITE(*,7) NF,NS,X(1),X(2),T,TM,TF
...
CC      IF(NCOUNT.GT.1) WRITE (4,8) NOR,NCOUNT,T,TP
